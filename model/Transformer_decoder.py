...
class SelfAttentionLayer(nn.Module):
    ...
    def forward_post(self, tgt,
        ...
        if tgt_mask:
            print(f'tgtmask SIZE:{tgt_mask.shape}')
        tgt2, attn_output_weights = self.self_attn(tgt, tgt, value=tgt, attn_mask=tgt_mask,
                              key_padding_mask=tgt_key_padding_mask)
        tgt = tgt + self.dropout(tgt2)
        tgt = self.norm(tgt) ### normalize after self_attention
    ...
    def forward_pre(self, tgt,
                    ...
                    tgt_key_padding_mask: Optional[Tensor] = None,
                    query_pos: Optional[Tensor] = None):
        tgt2 = self.norm(tgt) ### normalize before self_attention
        q = k = self.with_pos_embed(tgt2, query_pos)
        tgt2, attn_output_weights = self.self_attn(tgt2, tgt2, value=tgt2, attn_mask=tgt_mask,
                              key_padding_mask=tgt_key_padding_mask)
...
class CrossAttentionLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory_key, memory_value,
        ...
        tgt = tgt + self.dropout(tgt2)
        tgt = self.norm(tgt) ### normalize after cross attention
    ...
    def forward_pre(self, tgt, memory_key, memory_value,
                    ...
                    query_pos: Optional[Tensor] = None,
                    value_pos: Optional[Tensor] = None):
        tgt2 = self.norm(tgt) ### normalize before cross attention
...
class transformer_decoder(nn.Module):
  ...
  def __init__(self, args, hidden_dim, dim_feedforward, nheads=4, num_layers=1, pre_norm=False):
    super().__init__()
  ...
  def forward(self, image_embedding, prompt_embedding, attn_mask): 
    # we want to adjust the prompt embeddings
    bs, C, H, W = image_embedding.shape                                                                                                                                  
                                                                                                                                                                             
    image_pe = self.pe_layer(image_embedding, None).flatten(2).to(image_embedding.device).permute(2, 0, 1)  # (H*W, batch_size, channels)                                                                                         
    image_embedding = image_embedding.flatten(2).permute(2, 0, 1)                                                                                                                                
                                                                                                                                                                   
    prompt_embedding = prompt_embedding.permute(1, 0, 2)
    
    for i in range(self.num_layers):
        output, attn_weights = self.transformer_self_attention_layers_0[i](
            prompt_embedding, 
            tgt_mask=attn_mask,
