...
class GenerateReferencePipelineOptions:
	...
	def initialize(self):
		...
		# ================== input & output ==================
		self.parser.add_argument('--original', type=str, default="example/input/faceswap/original.jpg",help='Path to the original image')
		self.parser.add_argument('--original_mask', type=str, default="",help='Path to the original mask')
    	
		self.parser.add_argument('--verbose', default=False, type=bool, help='Whether to show the intermediate results')
		self.parser.add_argument('--output_dir', type=str, default="example/output/faceswap/在Wr+空间随机数融合",help='Path to the original mask')
